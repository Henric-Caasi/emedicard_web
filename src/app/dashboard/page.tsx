...
// NEW: Import the new activity log component
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <nav className="bg-white shadow-md w-full sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-5">
            ...
            {/* NEW: Add the Activity Log dropdown here */}
      ...
      <main className="max-w-6xl mx-auto py-10 px-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-4 mb-10">
          <div className="bg-white border rounded-lg p-4 text-center"><div className="text-xl text-black font-bold">6</div><div className="text-xs text-gray-500">Today Pending</div></div>
          <div className="bg-white border rounded-lg p-4 text-center"><div className="text-xl text-black font-bold">18</div><div className="text-xs text-gray-500">Total Pending</div></div>
          <div className="bg-white border rounded-lg p-4 text-center"><div className="text-xl text-black font-bold">10</div><div className="text-xs text-gray-500">Approved</div></div>
          <div className="bg-white border rounded-lg p-4 text-center"><div className="text-xl text-black font-bold">7</div><div className="text-xs text-gray-500">Disapproved</div></div>
          <div className="bg-white border rounded-lg p-4 text-center"><div className="text-xl text-black font-bold">24</div><div className="text-xs text-gray-500">Total Approved Today</div></div>
        </div>
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-6">
          <div className="flex gap-4 flex-col sm:flex-row">
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1">Select Health Card Type</label>
              <select value={selectedType} onChange={e => setSelectedType(e.target.value as keyof typeof applicants)} className="px-4 py-2 pr-10 border border-gray-300 text-black rounded-lg shadow-sm text-base focus:outline-none focus:ring-2 focus:ring-emerald-500">
            ...
            <div>
              ...
              <select className="px-4 py-2 pr-10 border border-gray-300 text-black rounded-lg shadow-sm text-base focus:outline-none focus:ring-2 focus:ring-emerald-500" value={filter} onChange={e => setFilter(e.target.value as Applicant["status"] | "")}>
        ...
        {selectedType !== adminAssignedType ? (
          <div className="flex flex-col items-center justify-center py-16">
            <span className="text-red-500 text-xl font-semibold mb-3">You're not assigned to validate this Health Card Type.</span>
            <span className="text-gray-500">Please select your assigned type (<b>{adminAssignedType} Card</b>) to view applicants.</span>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full bg-white rounded-xl shadow-md overflow-hidden">
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={4} className="text-center py-8 text-gray-400">No applicants found.</td></tr>
                ) : (
                  filtered.map((app: Applicant, i: number) => (
                    <tr key={i} className="border-b last:border-none">
                      ...
                      <td className="px-6 py-4"><span className={`text-white px-3 py-1 rounded-full text-xs font-semibold ${statusColors[app.status]}`}>{app.status}</span></td>
                      <td className="px-6 py-4"><Link href={`/dashboard/${encodeURIComponent(app.name)}/doc_verif`} className="text-emerald-600 underline font-semibold hover:text-emerald-800 text-sm">View</Link></td>
                    </tr>
                  ))
                )}
              </tbody>
